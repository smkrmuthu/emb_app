...
- Item names matter as much as the numbers — never substitute a different, unrelated dish name because the print is small or slightly blurry. Read the actual printed letters; if a few characters are genuinely illegible, transcribe the letters you can read plus "?" for the ones you can't (e.g. "SAMBAR VAD?I") rather than outputting a completely different, fluent-sounding item name — a confident wrong guess is worse than an honest partial read. If the bill's own header/name indicates a vegetarian restaurant ("Veg", "Pure Veg", etc.), item names must never come out as meat/fish/egg dishes — if your first read looks like one, that's a strong signal you misread it; look again for the vegetarian item it actually says.
